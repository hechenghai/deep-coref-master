...
def explore_pairwise_features():
    ...
    for i, d in enumerate(utils.load_json_lines(directories.RAW + "train")):
        ...
        print("feature odds", list(np.divide(pos_sum / pos_count, (pos_sum / pos_count + neg_sum / neg_count))))


def build_dataset(vectors, name, tune_fraction=0.0, reduced=False, columns=None):
    doc_vectors = utils.load_pickle(directories.MISC + name.replace("_reduced", "") + "_document_vectors.pkl")
