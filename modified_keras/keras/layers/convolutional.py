...
from keras.utils.theano_utils import shared_zeros, on_gpu
...
class MaxPooling1D(Layer):
    ...
    def get_output(self, train=False):
        X = self.get_input(train)
        X = T.reshape(X, (X.shape[0], X.shape[1], X.shape[2], 1)).dimshuffle(0, 2, 1, 3)
        output = pool.max_pool_2d_same_size(X, ds=self.pool_size, st=self.st, ignore_border=self.ignore_border)
...
class MaxPooling2D(Layer):
    ...
    def get_output(self, train=False):
        X = self.get_input(train)
        output = pool.max_pool_2d_same_size(X, ds=self.pool_size, st=self.stride, ignore_border=self.ignore_border)
        return output
